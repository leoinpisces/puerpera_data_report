...
#抓取apgar评分的正则表达式
G_apgar_searcher = re.compile(r'一评二评三评均\d{1,2}分|满分|一评\d{1,2}分|[1-3]分钟评分\d{1,2}分|[Aa]pgar评分均\d{1,2}分')
#抓取分娩日期的正则表达式
...
#判断新生儿是否死亡的正则表达式
G_fetus_dead_searcher = re.compile(r'新生儿死亡|死婴|死女婴|死男婴')
#抓取孕周的正则表达式
G_gestational_weeks_searcher = re.compile(r'\d{2}\+{0,1}[1-6]{0,1}周')
#抓取新生儿性别为“女”的正则表达式
G_female_searcher = re.compile(r'女婴|活女婴|女活婴')
#抓取新生儿性别为“男”的正则表达式
G_male_searcher = re.compile(r'男婴|活男婴|男活婴')
